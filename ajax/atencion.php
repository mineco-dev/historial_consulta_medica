		...
		$rspta=$atencion->listar();
 		...
 		while ($reg=$rspta->fetch_object()){
 			$data[]=array(
 				...
 				"6"=>$reg->num_documento,
 				"7"=>$reg->edad,
 				"8"=>($reg->estado=='Registrado')?'<span class="label bg-green">Registrado</span>':(($reg->estado=='Triaje')?'<span class="label bg-orange">Pre-Consulta</span>':'<span class="label bg-primary">Atendido</span>') 				
 				);
 		}
 		$results = array(
